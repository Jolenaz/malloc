...
void		init_page(t_page *page, size_t size, t_type type)
{
	...
	page->type = type;
	page->next = NULL;
	page->max_free_size = page->page_size - sizeof(t_block) - sizeof(t_page);
	page->first_block = (t_block*)(page + 1);
	init_block(page->first_block, page->max_free_size);
}
...
t_page		*add_page(size_t size, t_type type)
{
	...
	page = (t_page	*)mmap(0, size, PROT_READ |
				PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
	if (page == MAP_FAILED)
		return (NULL);
	init_page(page, size, type);
	ft_putstr("Create New page : ", g_debug_flag);
